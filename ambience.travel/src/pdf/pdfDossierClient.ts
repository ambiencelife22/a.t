...
function drawBodyText(
  doc: any, text: string, x: number, y: number, maxWidth: number,
  size = 9, color: RGB = THEME.ink, style: 'normal' | 'italic' = 'normal',
): number {
  sans(doc, style, size)
...
function drawBulletItem(doc: any, text: string, y: number, priority: DossierPriority, contentWidth: number): number {
...
export async function exportClientDossierPdf(dossier: ClientDossierData): Promise<void> {
  ...
  const [emblem, logo]: [Img | null, Img | null] = await Promise.all([
