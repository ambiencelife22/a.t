...
// same back-to-top button, same overflow-lock effect.
//
      ...
      >
        ...
        <nav style={{ flex: 1, padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto' }}>
          {hasNavItems && navItems!.map(item => (
            <GuideNavLink key={item.href + item.label} item={item} onNavigate={() => setDrawerOpen(false)} />
          ))}
        </nav>
      </div>
...
function GuideNavLink({ item, onNavigate }: NavLinkProps) {
