...
export function DiningGuideFilters({
  ...
  function handleAll() {
    onChange({ cuisines: new Set(), michelinOnly: false, highlightedOnly: false })
  }
  ...
  function toggleHighlighted() {
    onChange({ ...state, highlightedOnly: !state.highlightedOnly })
  }
  ...
  return (
    <nav style={filtersStyle} aria-label="Filter dining venues">
      ...
      {hasHighlightedItems && (
        <Chip active={state.highlightedOnly} onClick={toggleHighlighted}>
          Highlighted
        </Chip>
      )}
