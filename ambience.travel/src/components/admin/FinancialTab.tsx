...
//   Engagement (#admin/finance/engagement/<id>)   — drill-in: margin card + expense list
//
// Design: dark admin theme (A tokens). Cinematic where cinematic, quiet where quiet.
...
function MetricCard({ label, value, sub, color }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div style={{
      ...
      {sub && <div style={{ fontSize: 11, color: A.muted, fontFamily: A.font }}>{sub}</div>}
    </div>
  )
}
...
function SectionHeader({ title, action }: { title: string; action?: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 20 }}>
      <div>
        <div style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: A.gold, fontWeight: 700, fontFamily: A.font, marginBottom: 4 }}>Finance</div>
...
const EXPENSE_TYPES = [
  'Amenity', 'Dining', 'Entertainment', 'Gifting', 'Ground Transport',
  'Hospitality', 'Research', 'Telephone', 'Travel', 'Other',
]
...
function AddExpenseModal({
  ...
  async function handleSave() {
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : 'Failed to create expense')
    }
    setSaving(false)
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: A.bgCard, border: `1px solid rgba(216,181,106,0.25)`,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <Field label='Type'>
            <select style={inputStyle} value={form.expense_type} onChange={e => setForm(f => ({ ...f, expense_type: e.target.value }))}>
              {EXPENSE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </Field>
          <Field label='Billing Status'>
            <select style={inputStyle} value={form.billing_status} onChange={e => setForm(f => ({ ...f, billing_status: e.target.value as BillingStatus }))}>
...
function ExpenseRow({
  ...
  const isCurrency = expense.currency !== 'USD'
    ...
    <div style={{
      ...
      <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {expense.items.length > 0 && (
            <button
              ...
              style={{ marginTop: 6, background: 'none', border: 'none', color: A.muted, fontSize: 11, fontFamily: A.font, cursor: 'pointer', padding: 0, letterSpacing: '0.02em' }}
            >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: A.text, fontFamily: A.font, letterSpacing: '-0.02em' }}>
            {usd(expense.total_amount)}
          </div>
          {isCurrency && (
            <div style={{ fontSize: 10, color: A.faint, fontFamily: A.font }}>{expense.currency}</div>
          )}
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            {expense.billing_status === 'billable' && (
              <button disabled={acting} onClick={() => handleLifecycle(() => markBilled(expense.id), 'Marked as billed')} style={{ ...btnGhost, fontSize: 11, padding: '5px 10px', color: '#FBBF24', borderColor: 'rgba(251,191,36,0.3)' }}>Mark Billed</button>
            )}
            {expense.billing_status === 'billed' && (
              <button disabled={acting} onClick={() => handleLifecycle(() => markPaid(expense.id), 'Marked as paid')} style={{ ...btnGhost, fontSize: 11, padding: '5px 10px', color: '#4ade80', borderColor: 'rgba(74,222,128,0.3)' }}>Mark Paid</button>
            )}
            {(expense.billing_status === 'billable' || expense.billing_status === 'billed') && (
              <button disabled={acting} onClick={() => handleLifecycle(() => writeOff(expense.id), 'Written off')} style={{ ...btnGhost, fontSize: 11, padding: '5px 10px' }}>Write Off</button>
            )}
            {canDelete && (
              <button disabled={acting} onClick={handleDelete} style={{ ...btnDanger, padding: '5px 10px' }}>Delete</button>
            )}
          </div>
        </div>
      </div>
          ...
          {expense.items
            ...
            .map((item, i) => (
              <div key={item.id} style={{
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: A.text, fontFamily: A.font, flexShrink: 0 }}>
                  {usd(item.amount)}
                </div>
              </div>
            ))}
...
export function EngagementFinanceView({ engagementId }: { engagementId: string }) {
  ...
  async function load() {
    setLoading(true)
    try {
      const [{ expenses: exp }, sum] = await Promise.all([
        fetchExpensesByEngagement(engagementId),
        fetchEngagementSummary(engagementId),
      ])
      setExpenses(exp)
      setSummary(sum)
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Failed to load financials')
    }
    setLoading(false)
  }
  ...
  const grouped: Record<BillingStatus, Expense[]> = {
    absorbed:    [],
    billable:    [],
    billed:      [],
    paid:        [],
  ...
  for (const exp of expenses) {
    grouped[exp.billing_status]?.push(exp)
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
        <button
          onClick={() => navigateAdmin({ product: 'finance', tab: 'pipeline' })}
          style={{ background: 'none', border: 'none', color: A.muted, fontSize: 12, fontFamily: A.font, cursor: 'pointer', padding: 0, letterSpacing: '0.02em' }}
        >
          ← Pipeline
        </button>
      </div>
      ...
      {!loading && summary && (
        <>
          {/* Margin banner */}
          <div style={{
            background: A.bgCard, border: `1px solid ${margin >= 0 ? 'rgba(74,222,128,0.2)' : 'rgba(239,68,68,0.2)'}`,
            ...
            <div>
              ...
              <div style={{ fontSize: 36, fontWeight: 700, color: marginColor(margin), fontFamily: A.font, letterSpacing: '-0.03em' }}>
                {margin >= 0 ? '+' : ''}{usd(margin)}
              </div>
            </div>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <MetricCard label='Commission' value={usd(summary.total_commission)} sub={`${usd(summary.commission_received)} received`} />
              <MetricCard label='Absorbed' value={usd(summary.total_absorbed)} color={summary.total_absorbed > 0 ? '#ef4444' : A.text} />
              {summary.total_billable > 0 && <MetricCard label='Billable' value={usd(summary.total_billable)} color='#93C5FD' />}
              {summary.total_outstanding > 0 && <MetricCard label='Outstanding' value={usd(summary.total_outstanding)} color='#FBBF24' />}
              {summary.commission_outstanding > 0 && <MetricCard label='Comm. Outstanding' value={usd(summary.commission_outstanding)} color='#FBBF24' />}
            </div>
          </div>

          {/* Expense list by status group */}
          {expenses.length === 0 && (
            <div style={{ fontSize: 13, color: A.faint, fontFamily: A.font, padding: '16px 0' }}>No expenses recorded for this engagement.</div>
          )}
          ...
          {ORDER.map(status => {
            ...
            return (
              <div key={status}>
                <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: meta.color, fontFamily: A.font, marginBottom: 10 }}>
                  {meta.label} · {group.length}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {group.map(exp => (
                    <ExpenseRow key={exp.id} expense={exp} onAction={load} />
                  ))}
                </div>
              </div>
            )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {!loading && (
        <>
          ...
          {trips.length === 0 && (
            <div style={{ fontSize: 13, color: A.faint, fontFamily: A.font, padding: '16px 0' }}>No confirmed trips in the pipeline.</div>
          )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1, borderRadius: 12, overflow: 'hidden', border: `1px solid ${A.border}` }}>
            {/* Header */}
            <div style={{
              display: 'grid', gridTemplateColumns: '1fr 100px 100px 100px 100px 110px',
              padding: '10px 20px', background: A.bgCard,
              borderBottom: `1px solid ${A.border}`,
            }}>
              {['Trip', 'Commission', 'Received', 'Absorbed', 'Billable', 'Net Margin'].map(h => (
            ...
            {trips.map((trip, i) => (
              <button
                ...
                style={{
                  display: 'grid', gridTemplateColumns: '1fr 100px 100px 100px 100px 110px',
                  padding: '14px 20px', background: i % 2 === 0 ? A.bg : A.bgCard,
                  border: 'none', borderBottom: i < trips.length - 1 ? `1px solid ${A.border}` : 'none',
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: A.text, fontFamily: A.font, marginBottom: 2, textAlign: 'left' }}>
                    {trip.title ?? trip.url_id}
                  </div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    {trip.trip_code && (
                      <span style={{ fontSize: 10, color: A.faint, fontFamily: "'DM Mono', monospace" }}>{trip.trip_code}</span>
                    )}
                    {trip.start_date && (
                      <span style={{ fontSize: 10, color: A.muted, fontFamily: A.font }}>{fmtDate(trip.start_date)}{trip.end_date ? ` – ${fmtDate(trip.end_date)}` : ''}</span>
                    )}
                    {trip.status_slug && (
                      <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: A.gold, fontFamily: A.font }}>{trip.status_slug.replace(/_/g, ' ')}</span>
                    )}
                  </div>
                </div>
                {/* Numbers */}
...
// ── Default export — dispatches on tab ────────────────────────────────────────
